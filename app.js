    ...
    users.forEach(user => {
        if (user.email == mail) {
            if (user.password == pass) {
                ...
                if (user.email == 'admin@admin.in') { res.redirect('/admin') }
...
app.post('/user/update',async (req, res)=>{
    users = getDb().collection('users')
    await users.updateOne({_id:new ObjectId(req.session.user._id)},{$set:{password:'123'}})
    .then(async(result)=>{
    ...
    users.findOne({ email: req.user.emails[0].value })
        .then((user) => {
            if (user) {
                ...
                message = 'Account nor fount, Try signin'
                return res.redirect('/')
            }
        })
        .catch((error) => {
